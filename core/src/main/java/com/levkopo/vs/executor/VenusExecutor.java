...
public class VenusExecutor {
	...
	public VenusExecutor(Consumer<Throwable> throwableConsumer) {
	...
	protected Value run(Container container, ScriptMode mode, Supplier<Boolean> shouldRun) {
		try {
				...
				if (component instanceof Container) {
					if (component instanceof AsyncContainer) {
						...
						Thread thread = new Thread(() -> {
							VenusExecutor executor = new VenusExecutor(throwableConsumer);
					...
					} else if(component instanceof TryContainer){
						TryContainer c = (TryContainer) component;
						Consumer<Throwable> throwableConsumerLast = throwableConsumer;
						throwableConsumer = (t)->{
							throwableConsumer = throwableConsumerLast;
							try {
								FunctionCall caller = new FunctionCall("catch", new Constant(new StringValue(t.getMessage())));
								caller.resolve(new Context(c, context), context);
							}catch (ScriptRuntimeException ee){
								ee.printStackTrace();
								throwableConsumer.accept(t);
							}
						};
						result = run(c, mode, shouldRun);
						throwableConsumer = throwableConsumerLast;
